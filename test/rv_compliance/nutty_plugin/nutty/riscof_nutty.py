...
class nutty(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        runner_script = os.path.join(get_repo_path(), "nutty/targets/run_rv_compliance.py")
        self.dut_exe =  f"python {runner_script}"
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      ###---------------------- BUILD THE SOC ------------------------ ####
      from nutty.targets.build_rv_compliance_soc import  build_rv_compliance_soc
      build_rv_compliance_soc() # now we have the compiled soc that can be run using vvp
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + f" --elf {elf} --sig {sig_file}"
          else:
            simcmd = 'echo "NO RUN"'
